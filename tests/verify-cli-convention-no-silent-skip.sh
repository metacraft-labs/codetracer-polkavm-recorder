#!/usr/bin/env bash
# Verify that the codetracer-polkavm-recorder CLI complies with
...
#     `CODETRACER_POLKAVM_RECORDER_DISABLED` are referenced in source so
...
BIN="${REPO_ROOT}/target/debug/codetracer-polkavm-recorder"
if [[ ! -x "${BIN}" ]]; then
...
# ---------------------------------------------------------------------------
# `trace-ink` subcommand --help
# ---------------------------------------------------------------------------

TRACE_INK_HELP="$("${BIN}" trace-ink --help)"

assert_absent "--format" "trace-ink --help" "${TRACE_INK_HELP}"
assert_absent "CODETRACER_FORMAT" "trace-ink --help" "${TRACE_INK_HELP}"
assert_present "--out-dir" "trace-ink --help" "${TRACE_INK_HELP}"
...
assert_present "codetracer-polkavm-recorder" "--version output" "${VERSION_OUT}"

# ---------------------------------------------------------------------------
# Source-level reference for the env-var fallback
# ---------------------------------------------------------------------------

# The recorder must reference CODETRACER_POLKAVM_RECORDER_OUT_DIR in
# source (otherwise the env-var fallback either doesn't exist or has
# been silently removed).  We grep recursively under src/.
if ! grep -rqF "CODETRACER_POLKAVM_RECORDER_OUT_DIR" "${REPO_ROOT}/src"; then
  echo "FAIL: CODETRACER_POLKAVM_RECORDER_OUT_DIR must be referenced in src/" >&2
  exit 1
fi
echo "ok: CODETRACER_POLKAVM_RECORDER_OUT_DIR referenced in src/"

if ! grep -rqF "CODETRACER_POLKAVM_RECORDER_DISABLED" "${REPO_ROOT}/src"; then
  echo "FAIL: CODETRACER_POLKAVM_RECORDER_DISABLED must be referenced in src/" >&2
  exit 1
fi
echo "ok: CODETRACER_POLKAVM_RECORDER_DISABLED referenced in src/"
